...
Then open http://localhost:3000, and write an article with the form's sample values or your own.
...
- `app/page.tsx` is the form, a client component that calls the action through React's `useActionState`. `lib/pipelex.ts` holds the client, and only server code imports it.
