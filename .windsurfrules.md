...
- You should ALWAYS RUN the terminal command `pipelex validate all` when you are writing a `.plx` file. It will ensure the pipe is runnable. If not, iterate.
...
type = "PipeCondition"
description = "A conditonal pipe to decide whether..."
...
ALWAYS RUN `pipelex validate all` when you are finished writing pipelines: This checks for errors. If there are errors, iterate until it works.
