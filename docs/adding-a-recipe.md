...
Some doors need no code of the reader's own: a request to a chatbot or to a coding agent, or a few HTTP calls any tool can make. Such a recipe lives under the door it shows, such as `recipes/run/<name>/`, and follows the same rules as a code recipe, less its code and its generated types:
