...
- `scripts/checks.py` holds the checks that need no key, and `scripts/hosted.py` the calls to production. `scripts/tutorial.py` finds the tutorial's bundles, which `check-methods` validates one by one.
...
| `make check-methods` | `PIPELEX_API_KEY` | Every package validates on production from its files, and its contract snapshot is current; then every `.mthds` file under `tutorial/`, each a self-contained lesson with no manifest, validates on production alone, under its path from the repository root |
