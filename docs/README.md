...
| `make check-recipe-types` | The network, to install each recipe's dependencies, Node.js 22 with npm, and shellcheck | Every Python recipe script type-checks in strict mode, in its own environment, every TypeScript recipe package passes its codegen gate and `tsc --noEmit`, and every recipe's shell script passes shellcheck. Where shellcheck is not installed, the check says so and goes on; in CI, whose runner image carries it, its absence fails the check |
