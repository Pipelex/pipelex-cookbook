...
|---|---|---|
| `make check-render` | Nothing | Every committed page, snippet file and snippet sidecar equals a fresh render, and every directory under `tests/snippets/` belongs to a method |
...
| `make refresh` | `PIPELEX_API_KEY` | Rewrites every contract snapshot from production, renders, then rewrites every recipe's and page snippet's generated types from production |
