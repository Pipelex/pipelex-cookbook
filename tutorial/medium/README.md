...
**To change default models and presets**, edit `.pipelex/inference/deck/base_deck.toml`. See [Configure AI Providers](https://docs.pipelex.com/latest/get-started/configure-ai-providers/) for the providers you can use and the models each one serves, and the [Inference Backend Configuration](https://docs.pipelex.com/latest/configuration/config-technical/inference-backend-config/) documentation for details.
...
**Run it:**
```bash
pipelex run bundle tutorial/medium/1_model_config.mthds
```
...
**Run it:**
```bash
pipelex run bundle tutorial/medium/2_batch_processing.mthds
```
...
**Run it:**
```bash
pipelex run bundle tutorial/medium/3_parallel_execution.mthds
```
