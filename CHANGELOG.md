...
- **The cookbook now onboards on your own provider keys, not a Pipelex Gateway key (Breaking).** The quick start, the tutorials, the CSV example and the CrewAI example all told newcomers to set `PIPELEX_GATEWAY_API_KEY` from a free-credits signup; that key is no longer a product. They now ask for a key per provider, or a single `OPENROUTER_API_KEY` to reach models from many providers at once, and point at the provider-configuration documentation instead of the retired gateway-models page. `.env.example` carries `OPENROUTER_API_KEY` and an optional `PIPELEX_API_KEY` for running methods on the hosted Pipelex API rather than locally.
- **The shipped `.pipelex/` configuration now runs on OpenRouter instead of the gateway, and the onboarding says how to switch a backend on (Breaking).** The prose had moved to bring-your-own-keys while the committed configuration had not: `backends.toml` still carried a `[pipelex_gateway]` entry as the one backend switched on, `routing_profiles.toml` still had `active = "all_pipelex_gateway"`, and the example profiles still routed specific models there — so a reader who followed the new instructions, put a provider key in `.env` and ran an example got a method routed at a backend that is going away. The gateway backend, its profile and its model catalogs are gone. The active profile is now `cookbook_byok`, which sends language models to OpenRouter — one key for many providers — and routes document extraction, image generation and web search to the provider that serves each, since OpenRouter serves language models only; the backends those routes name are switched on, and the `hello-1` plugin-demo route moved onto the new profile so `using_inference_plugins` still runs with no key. The README, the tutorial README and `.env.example` now say that a key alone is not enough for any provider other than OpenRouter and link to the provider-configuration documentation rather than restating it.
- **The README now presents the hosted Pipelex API as a first-class option, by name.** Running methods without holding provider keys was a single trailing sentence saying to run `pipelex login`, which never told the reader that the hosted API exists, that they sign up for it at [app.pipelex.com](https://app.pipelex.com/), or that a `PIPELEX_API_KEY` is what comes back. It is now one of the two labelled options in the setup step, and `.env.example` describes that key as the alternative to a key per provider rather than marking it `[OPTIONAL]`.
- **The extraction default and the image-generation gate no longer depend on the gateway's keyless catalog.** The gateway backend fetched its model catalog from Pipelex's servers with no credential, which is what let the shipped deck resolve `azure-document-intelligence` and `nano-banana-pro` in an environment holding no keys at all — including CI. Without it, `x_custom_extract_deck.toml` points the extraction aliases, and the handle the vendored `documents` method names directly, at `mistral-ocr`, which the Mistral catalog here does carry. `design_slides` is excluded from the static validation gate for the same reason: it names an image-generation model directly, and no image-generation model resolves without a provider credential. It is reported as a skip with that reason, and validates locally once `GOOGLE_API_KEY` is set.
- **Documentation links point at paths that resolve.** The `configure-ai-providers` and `inference-backend-config` links were missing the `/latest/` version prefix that mike inserts, or used a retired `/home/7-configuration/` path, so every one of them 404'd. The deck and telemetry configuration files also stopped describing gateway behaviour that no longer exists.
