...
- **The research report** is its own method, `research_report`, which the CrewAI example now loads from `methods/research_report/`: a fact-check's verdict is one of three fixed values with a numeric confidence, and the report's footer reads "Generated by Pipelex".
...
- **Synthetic expense data generation** labels each expense as legitimate or with its fraud pattern and gives every expense its own id, where they were all `EXP-<date>-0001`; its PDF export script is gone, and with it the `weasyprint` dependency.
- **`make check-links` fetches every sample URL, wherever it is hosted**, following redirects, and a request that loops or names a malformed URL is reported as broken instead of crashing the check. A link into the cookbook counts as not yet published only when it answers 404; any other failure is broken. A method whose input takes several files links each one on its page, a list output lists its items' fields, and a string holding double quotes is quoted in the Python snippet as ruff formats it.
