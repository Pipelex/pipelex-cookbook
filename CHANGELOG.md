...
- **Method pages**: every cookbook method now lives in `methods/<name>/` as a package runnable by address (`github.com/Pipelex/pipelex-cookbook/<name>@<tag>`), with a manifest, sample inputs linked by URL, an answer key and a generated `README.md` page. The page shows every way to use the method: in a chatbot through the Pipelex MCP, in TypeScript, Python or any HTTP client, as an app from the method-app initializer, as a method of your own, and on your own machine.
...
- **Gantt chart extraction is a method page, and it now extracts milestones**: the example moved from `examples/b_basics/document_extract/extract_gantt/` to `methods/extract_gantt/`, with its sample in `assets/extract_gantt/`. It returned an empty `milestones` list on every chart, because no step read them; a new step lists each milestone with its date, and the chart is assembled without a model call.
- **Ten more examples are method pages, runnable by address**: DPE extraction, generic document extraction, slide deck extraction, document question answering, the advisory board, blog article generation, the research report, the Discord newsletter, synthetic data generation and synthetic expense data generation moved to `methods/<name>/`, each with a sample, an answer key it met on a production run, and a generated page. To run on the hosted API, their Python structure classes became MTHDS concepts, the library pipe some of them borrowed is now part of the method, the retired `@best-gemini`, `@best-gpt` and `$synthesizing-data` models gave way to production presets, and every file sample moved to `assets/<name>/` or kept its public URL.
- **Energy diagnostic (DPE) extraction** returns the estimated yearly energy costs as a low and a high figure instead of one number, since a DPE always gives a range, and its domain and main pipe are renamed `dpe` and `extract_dpe`.
...
- **Document question answering** always returns a `DocumentAnswer` instead of an output type chosen by the caller (the `ReferenceCount` concept is gone), and `context` is an optional input instead of empty text.
...
- **The research report** is its own method, `research_report`, which the CrewAI example now loads from `methods/research_report/`: a fact-check's verdict is one of three fixed values with a numeric confidence, the report's footer reads "Generated by Pipelex", and its description says it drafts from the model's own knowledge without searching any source, where it claimed to research and fact-check.
...
- **`make check-links` fetches every sample URL, wherever it is hosted**, following redirects and asking again with GET when a host answers HEAD with an error, and a request that loops or names a malformed URL is reported as broken instead of crashing the check. A link into the cookbook counts as not yet published only when it answers 404; any other failure is broken. A method whose input takes several files links each one on its page, a list output lists its items' fields, and a string holding double quotes is quoted in the Python snippet as ruff formats it.
...
- **The expense validation example's script** loaded the expense generator's bundle instead of its own and named a sample PDF under `assets/expense_reports/`, which does not exist; it now loads `examples/wip/validate_expense_data/` and names `assets/expense_report/sample_expense_report.pdf`. It still cannot complete a run, since it sends a `document` where its bundle asks for `expense_pdf` and `receipts`, and this work-in-progress example is due to be retired rather than repaired.
