...
- **The runtime's local files stay out of git**: `.pipelex/`, where the runtime writes the traces of a tutorial lesson run from the clone's root, is ignored again now that the cookbook carries no configuration there.
- **The tutorial's setup follows the runtime's own**: it installs Pipelex with `pipelex init` and `pipelex doctor`, gives the runtime access to models through Configure AI Providers rather than a gateway key, clones the cookbook before the first run, names the editor extension, and points model settings at `~/.pipelex`. The model lesson names a model by the alias `@default-premium` rather than the retired `best-claude`, and sets no temperature, and the dead links to the Pipe Builder, to retired documentation pages and to the old examples are gone.
