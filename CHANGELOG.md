...
- **Bump `pipelex` to `0.30.0`.** See the [Pipelex changelog](https://docs.pipelex.com/latest/changelog/) for the full set of changes. Pinned to git rev `8b99246` pending PyPI publication.
- **Adapt to domain-prefixed structure class names.** Pipelex's concept registry now stores structure class names as `domain__ConceptCode` (qualified) rather than the bare `ConceptCode`. Regenerated every example's autogenerated structure files (`validate_expense_data`, `gen_expense_data`, `discord_newsletter`, `answer_from_documents`) so each class definition and cross-reference uses the qualified name. Updated `examples/wip/validate_expense_data/funcs.py` and the affected `run_*.py` scripts to import and reference the qualified names. Concept-ref string literals (e.g. `"document_qa.ReferenceCount"`) stay unprefixed since concept refs use bare codes.
- **Adapt to stricter template sigil rule.** Pipelex now rejects inline `@var` in PipeLLM prompts when `var` collides with a declared input — the `@` sigil produces tag-wrapped block content and must appear alone on its own line. Converted inline references to `$var` in `examples/b_basics/document_extract/answer_from_documents/bundle.mthds` and `examples/c_advanced/crewai_with_pipelex_tools/research_report.mthds`.
- **Remove invalid `structuring_method` directive.** Pipelex now rejects `structuring_method = "preliminary_text"` on a `PipeLLM` whose output is `Text` (the directive only makes sense for structured outputs, since it elaborates into a `PipeLLM` → `PipeStructure` sequence). Removed it from `write_markdown_from_page_content_dpe` in `examples/b_basics/document_extract/extract_dpe/bundle.mthds`, which already emits Text and relies on the downstream `conclude_dpe` pipe for structuring.
