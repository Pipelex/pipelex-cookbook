...
- DO NOT code too much. DO the strict minimum so that it works, its clear and straightforward.
- DO not hallucinate: If you don't have an answer, say that you need more information, and ask a question.
...
- First establish a plan to solve the problem, then transcribe it in pipelex. If you need to use mutliple steps pipeline, do it.
extract_cv_info
...
class YourModel(StructuredContent): # Always be a subclass of StructuredContent
    """Brief model description."""  # Keep docstrings minimal - they appear in LLM prompts. Include only relevant information needed for the inference
    ...
    field3: Optional[str] = None  # Avoid verbose Field descriptions: Include only relevant information needed for the inference
...
**CRITICAL**: Keep descriptions and docstrings minimal in StructuredContent classes. These are automatically included in LLM prompts, so verbose descriptions bloat context and increase costs. Only include essential information.
## Usage
...
- Keep docstrings and Field descriptions minimal - they appear in LLM prompts
- Only use Field() when needed for default_factory or validation, not for descriptions
...
inputs = { document = "PDF" } # or { ocr_input = "Image" } if its an image. This is the only input
output = "Page"
...
Important:
The structure of the output concept will be automatically injected in the prompt template.
Therefore, do NOT inclure fields descriptions in the prompt template.
...
Then, ALWAYS create a Python execution file to demonstrate how to use the pipeline.
But don't write documentation unless asked explicitly to.

# Pipe Base Library

Pipelex already holds în a base library pipelines that you can reuse. 
- `extract_page_contents_from_pdf`: This is an OCR pipe, that has as `inputs = { document = "PDF" }` and `output = "Page"`. 
- `generate_image`: Generate an image, `inputs = { prompt = "ImgGenPrompt" }`, `output = "Image"`

VERY IMPORTANT:
- Do NOT do too much. DO NOT do more than you are asked to do.
- DO NOT add doc strings to BaseModels excepts if necessary information is added. Same for the descriptions of the fields.
- You can use a PipeFunc to combine BaseModels into a new Output structure concept.
- Always run `make cc` before finishing the pipeline.
- When writing the example python file to run the pipeline, be straightforward, DO NOT add any extra code, useless prints etc
